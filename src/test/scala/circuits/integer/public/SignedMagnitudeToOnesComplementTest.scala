...
class SignedMagnitudeToOnesComplementTest extends AnyFlatSpec with ChiselSim {

  behavior of "SignedMagnitudeToOnesComplement"
  ...
  for ((width, value) <- testInputs) {
    it should s"convert $value correctly for width $width" in {
      simulate(new SignedMagnitudeToOnesComplement(width)) { dut =>
