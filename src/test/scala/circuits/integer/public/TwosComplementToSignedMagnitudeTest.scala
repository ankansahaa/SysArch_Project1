...
class TwosComplementToSignedMagnitudeTest extends AnyFlatSpec with ChiselSim {

  behavior of "TwosComplementToSignedMagnitude"
  ...
  for ((width, value) <- testInputs) {
    it should s"convert $value correctly for width $width" in {
      simulate(new TwosComplementToSignedMagnitude(width)) { dut =>
