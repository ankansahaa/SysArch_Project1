...
class OnesComplementToSignedMagnitudeTest extends AnyFlatSpec with ChiselSim {

  behavior of "OnesComplementToSignedMagnitude"
  ...
  for ((width, value) <- testInputs) {
    it should s"convert $value correctly for width $width" in {
      simulate(new OnesComplementToSignedMagnitude(width)) { dut =>
        val inputBits    = OnesComplement.bigIntToOnesComplement(value, width)
        val expectedBits = SignedMagnitude.bigIntToSignedMagnitude(value, width)
            ...
            .expect(
              ...
              s"Bit $i did not match expected value for input $value with width $width.\n" +
                ...
                s"but got  ${(for (i <- 0 until width)
                    yield if (dut.signedMagnitude(i).peek().litToBoolean) "1" else "0").mkString}.\n"
            )
        }
      }
    }
