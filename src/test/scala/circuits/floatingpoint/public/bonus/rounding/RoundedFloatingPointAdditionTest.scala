...
import sysarch.circuits.floatingpoint.bonus.rounding.RoundedFloatingPointAddition
import sysarch.circuits.util.FloatingPoint

class RoundedFloatingPointAdditionTest extends AnyFlatSpec with ChiselSim {
  ...
  val testInputs = Seq(
    ...
    (-17277776.0, -1.0, -17277776.0),
    (16777218.0, 1.0, 16777220.0),
    (16777218.0, 2.0, 16777220.0),
    (16777218.0, 3.0, 16777220.0),
    (16777218.0, 4.0, 16777222.0)
  )

  for ((a, b, expected) <- testInputs) {
    it should s"add $a and $b correctly" in {
      simulate(new RoundedFloatingPointAddition) { dut =>
        pokeFloat(dut.a, a)
        pokeFloat(dut.b, b)
